...
def plot_interconnector_summary(n, folder):
    ...
    ax1.bar(x + width/2, df["Annual exports from DK [GWh]"], width, label='Exports from DK', color='orange', alpha=0.8)
...
def analyze_denmark_export_origin(n, folder):
    ...
    ax.bar(x, df["Re-exported imports [GWh]"], bottom=df["Exports from DK surplus [GWh]"], label='Re-exported imports', color='navajowhite', alpha=0.9)
